...
/*		if (pATEInfo->OneSecPeriodicRound % 4 == 0) */
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("Channel is %u, and Tx power = 0x%02x\n", pATEInfo->Channel, (pAd->TxPower[pATEInfo->Channel-1].Power)));			  
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (pAd->ate.bQATxStart == FALSE)  
	{
		...
		snprintf(IPGStr, sizeof(pAd->ate.IPG), "%u", pAd->ate.IPG);
	...
	if (1)
	{
		...
		if (IS_RT3352(pAd) || IS_RT5350(pAd))
		{
			...
			if (RFValue2 > RFValue)
			{
				...
				{
					RT30xxWriteRFRegister(pAd, RF_R17, (UCHAR)(RFValue + offset + 0x80));
					RtmpOsMsDelay(1);
				}
			}
			else
	...
	if (IS_RT5350(pAd))
	{
		...
		for (i = 0; i < 500; i++)
		{
			...
			{ 
				...
				/* If TSSI reading is not within 0x0~0x7C, then treated it as 0x7C. */
				if (TssiInfo0 > 0x7C)
				{
					DBGPRINT(RT_DEBUG_TRACE, ("TSSI: BBP_R49=%X is native value\n", TssiInfo0));
					*pBbpR49 = TssiInfo0 = 0x7C;
					continue;
				}
				else
				{
