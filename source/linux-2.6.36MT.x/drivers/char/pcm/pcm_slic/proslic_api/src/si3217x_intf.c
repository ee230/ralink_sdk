...
**
** $Id: si3217x_intf.c,v 1.1 2011-11-14 07:54:36 qwert Exp $
**
** SI3217X_Intf.c
        ...
        isVerifiedProslic(pProslic[k]);     /* Populate channelType parameter in channel struct */
            ...
            if (pProslic[k]->debugMode){
                if(pProslic[k]->channelType == PROSLIC)
                {
					...
					LOGPRINT("SLIC CH[%d] MSTRSTAT=0x%02X\n",k, data);
                ...
                {
                	data = pProslic[k]->ReadRegX(pProslic[k]->pProHWX,pProslic[k]->channel,MSTRSTAT);
                    LOGPRINT("si3217x : Channel %d : Type = UNKNOWN(type=%02X)\n",pProslic[k]->channel,pProslic[k]->channelType);
                    LOGPRINT("SLIC CH[%d] UNKNOWN , MSTRSTAT=0x%02X\n",k, data);
                    data = pProslic[k]->ReadRegX(pProslic[k]->pProHWX,pProslic[k]->channel,ID);
                    LOGPRINT("si3217x : Channel %d : ID =%02X\n",pProslic[k]->channel,data);
