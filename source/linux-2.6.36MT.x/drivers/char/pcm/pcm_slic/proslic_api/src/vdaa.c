...
**
** $Id: vdaa.c,v 1.1 2011-11-14 07:54:36 qwert Exp $
**
** Vdaa.c
...
int Vdaa_Init (vdaaChanType_ptr *pVdaa,int size){
	...
	printk("DAA:[SYS_LINE_DEV_REV]=0x%02X chipRev=%d IsType=%d\n",data,pVdaa[k]->deviceId->chipRev,pVdaa[k]->deviceId->lsType);
		...
		if (pVdaa[k]->channelEnable) {
            ...
            data = pVdaa[k]->ReadRegX(pVdaa[k]->pVdaaHWX,pVdaa[k]->channel,PCMRX_CNT_LO);
            		printk("DAA:[PCMRX_CNT_LO]=0x%02X\n",data);				
			if (data != 0x5A){
