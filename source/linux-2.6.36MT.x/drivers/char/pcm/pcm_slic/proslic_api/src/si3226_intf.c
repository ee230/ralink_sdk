...
**
** $Id: si3226_intf.c,v 1.1 2011-11-14 07:54:36 qwert Exp $
**
** SI3226_Intf.c
...
int Si3226_Init (proslicChanType_ptr *pProslic, int size){
	...
	*/
	...
	for (k=0;k<size;k++){
		for(i=0;i<3;i++)
		{
    	data = pProslic[k]->ReadRegX(pProslic[k]->pProHWX,pProslic[k]->channel,MSTRSTAT);
    	printk("SLIC CH[%d] MSTRSTAT=0x%02X\n",k, data);
    	pProslic[k]->DelayX(pProslic[k]->pProTimerX,50);
    	}
    }
    /*
...
int Si3226_VerifyPatch (proslicChanType *pProslic, const proslicPatch *pPatch){ 
    ...
    for (loop=0; loop<1024; loop++){
		if (pPatch->patchData[loop] != 0){
			...
			if (pPatch->patchData[loop]<<9 != ramdata){
				printk("\npatchData[%d]=%02X != %02X\n",loop,pPatch->patchData[loop]<<9, ramdata);
				//loop = 1024;			
				err = 1;
	...
	for (loop=0;loop<8;loop++){
		...
		if (pPatch->patchEntries[loop] != 0){
			...
			{
					printk("\nE1:patchEntry[%d]=%02X != %02X\n",loop,pPatch->patchData[loop]&0xff, data);
					err = 1;
			}
			data = ReadReg (pProHW, pProslic->channel, jmp_table+1);
			if (data != (pPatch->patchEntries[loop]>>8))
			{
					printk("\nE2:patchEntry[%d]=%02X != %02X\n",loop,pPatch->patchData[loop]>>8, data);
					err = 1;
			}
		}
		jmp_table+=2;
