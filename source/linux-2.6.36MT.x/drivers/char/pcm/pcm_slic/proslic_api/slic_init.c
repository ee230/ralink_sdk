...
int ProSLIC_HWInit(void)
{
	...
	*/
	...
	//ProSLIC_GetLBCalResultPacked(arrayOfProslicChans[0], &result);
	//printk("LBCal=0x%08X\n");
	//result = 0x78;
	//ProSLIC_LoadPreviousLBCalPacked(arrayOfProslicChans[0], &result);
	//result = 0x78;
	//ProSLIC_LoadPreviousLBCalPacked(arrayOfProslicChans[1], &result);
	/*
	...
	*/
	for(i=0;i<NUMBER_OF_CHAN;i++)
	{
		...
		ProSLIC_PCMTimeSlotSetup(ports[i].ProObj,1+i*16,1+i*16);
