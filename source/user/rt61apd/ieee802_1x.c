...
static void ieee802_1x_send(rtapd *rtapd, struct sta_info *sta, u8 type, u8 *data, size_t datalen)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,"Send to Sta(raL%d) with Identifier %d\n",sta->ApIdx,*(data+1));
	memset(buf, 0, len);
...
static void handle_eap(struct sta_info *sta, u8 *buf, size_t len)
{
	...
	DBGPRINT(RT_DEBUG_INFO," Receive EAP: code=%d identifier=%d length=%d from Supplicant raL%d\n",
...
void ieee802_1x_receive(rtapd *rtapd, u8 *sa, u8 *apidx, u8 *buf, size_t len, u16 ethertype)
{
	...
	switch (hdr->type)
	{
		case IEEE802_1X_TYPE_EAP_PACKET:
			DBGPRINT(RT_DEBUG_TRACE,"Handle EAP_PACKET from raL%d\n",sta->ApIdx);
			handle_eap(sta, (buf + LENGTH_8021X_HDR), datalen);
			break;

		case IEEE802_1X_TYPE_EAPOL_START:
			DBGPRINT(RT_DEBUG_TRACE,"Handle EAPOL_START from raL%d\n",sta->ApIdx);
		...
		case IEEE802_1X_TYPE_EAPOL_ENCAPSULATED_ASF_ALERT:
			/* TODO: */
			DBGPRINT(RT_DEBUG_TRACE,"Handle EAPOL_ALERT from raL%d\n",sta->ApIdx);
			break;

		default:
			DBGPRINT(RT_DEBUG_TRACE,"Handle Unknown EAP message(Type:%d) from raL%d\n",hdr->type,sta->ApIdx);
			break;
	}
