...
#if defined ENABLE_FEATURE_TFTP_PROGRESS_BAR
	off_t pos;
	off_t size;
	const char *file;
	bb_progress_t pmt;
...
# define remote_file NULL
		...
		USE_FEATURE_TFTP_BLOCKSIZE(, int want_transfer_size)
...
#define socket_fd (pfd[0].fd)
	int len;
	int send_len;
	USE_FEATURE_TFTP_BLOCKSIZE(smallint expect_OACK = 0;)
	smallint finished = 0;
	uint16_t opcode;
	uint16_t block_nr;
	uint16_t recv_blk;
...
# endif
	result = tftp_protocol(
		...
		local_file, remote_file
		USE_FEATURE_TFTP_BLOCKSIZE(, 1 /* want_transfer_size */)
...
int tftpd_main(int argc UNUSED_PARAM, char **argv)
{
	...
	USE_FEATURE_TFTP_BLOCKSIZE(int want_transfer_size = 0;)
...
# endif
 ...
 do_proto:
	...
	result = tftp_protocol(
		...
		USE_FEATURE_TFTP_BLOCKSIZE(, want_transfer_size)
		USE_FEATURE_TFTP_BLOCKSIZE(, blksize)
	);
