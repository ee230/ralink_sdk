...
#define __sysRegRead(phys)        \
        (*(volatile unsigned int *)PHYS_TO_K1(phys))
#define __sysRegWrite(phys,val)   \
...
asmlinkage void __init start_kernel(void)
{
	...
	//int regValue = __sysRegRead(SYSCTL_ADDR);

	//regValue = (regValue&(~(0x2)));
	//__sysRegWrite(SYSCTL_ADDR, regValue);
