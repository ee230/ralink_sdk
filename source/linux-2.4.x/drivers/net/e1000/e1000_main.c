...
e1000_sw_init(struct e1000_adapter *adapter)
{
	...
	spin_lock_init(&adapter->sw_r_lock);
	spin_lock_init(&adapter->sw_t_lock);
...
void skb_dump1(struct sk_buff *sk)
{
    unsigned int i;
 ...
 */
	 ...
	 //printk("skb->head = %x, skb->data= %x, skb->tail = %x, skb->end = %s\n", skb->head, skb->data, skb->tail, skb->end);
        ...
        {
        if((*(int*)(skb->head)) == 0x28800021) {
                printk("eth0:this packet interested by nat alg\n");
                printk("SRC: %u.%u.%u.%u: %u\n", NIPQUAD(ip->saddr), tcp->source);
                printk("DST: %u.%u.%u.%u: %u\n", NIPQUAD(ip->daddr), tcp->dest);
                printk("********************************\n");

                (*(int*)(skb->head)) = 0;

//              if(tx_skb_cnt!=0 && tx_skb_cnt!=1)
//                printk("[%d] ", tx_skb_cnt);
        }else{
                //printk("!voip ");
        }
...
e1000_clean(struct net_device *netdev, int *budget)
{
	...
	int tx_cleaned, flags;
	int work_done = 0;
	
	spin_lock_irqsave(&adapter->clean_lock, flags);
	tx_cleaned = e1000_clean_tx_irq(adapter);
	e1000_clean_rx_irq(adapter, &work_done, work_to_do);
	spin_unlock_irqrestore(&adapter->clean_lock, flags);
