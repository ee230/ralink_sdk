...
static _INLINE_ unsigned int serial_in(struct async_struct *info, int offset)
{
	...
	default:
#ifdef CONFIG_UART_RT2880
		/* no byte access of surfboard chip registers */
		return (*(int*)(mips_io_port_base + (info->port + offset)));
#else
...
#ifdef CONFIG_UART_RT2880
		/* no byte access of surfboard chip registers */
		*(int*)(mips_io_port_base + (info->port + offset)) = value;
#else
...
#ifdef CONFIG_UART_RT2880
	serial_outp(info, UART_DLL, quot);  /* no byte accesses for surfboard */
#else
...
{
	unsigned char old_fcr, old_mcr;
#ifdef CONFIG_UART_RT2880
	unsigned int old_dll;
#else
				      ...
				      unsigned long flags)
{
	...
	unsigned int scratch5, scratch6;
#else
	...
	serial_outp(info, UART_LCR, UART_LCR_DLAB);
#ifdef CONFIG_UART_RT2880
	scratch6 = serial_inp(info, UART_DLL);
#else
	...
	scratch5 = serial_inp(info, UART_DLL);
	scratch2 = (unsigned char)(scratch5 & 0xff);
	scratch = (unsigned char)((scratch5 & 0xff00) >> 8);
#else
	serial_outp(info, UART_DLL, 0);
	...
	serial_outp(info, UART_LCR, UART_LCR_DLAB);
#ifdef CONFIG_UART_RT2880
	serial_outp(info, UART_DLL, scratch6);
#else
