...
struct mtd_info *cfi_cmdset_0701(struct map_info *map, int primary)
{
	...
	if (cfi->cfi_mode==CFI_MODE_CFI){
		...
		printk(KERN_NOTICE " SST/Fujitsu Extended Query Table v%c.%c at 0x%4.4X\n",
		       major, minor, adr);
		cfi_send_gen_cmd(0xaa, 0x5555, base, map, cfi, cfi->device_type, NULL);
		cfi_send_gen_cmd(0x55, 0x2aaa, base, map, cfi, cfi->device_type, NULL);
...
static struct mtd_info *cfi_sststd_setup(struct map_info *map)
{
	...
	if (cfi->fast_prog) {
		/* In cfi_sststd_write() we frob the protection stuff
		...
		printk(KERN_NOTICE "cfi_cmdset_0701: Disabling fast programming due to code brokenness.\n");
		cfi->fast_prog = 0;
	}
...
static int do_write_oneword(struct map_info *map, struct flchip *chip, unsigned long adr, __u32 datum, int fast)
{
 ...
 // printk("\n kaiker ,do_write_oneword adr=%08X ,datum=%X,fast=%d\n",adr,datum,fast);
