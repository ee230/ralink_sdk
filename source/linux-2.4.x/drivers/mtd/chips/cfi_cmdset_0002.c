...
static struct mtd_info *cfi_amdstd_setup(struct map_info *map)
{
	...
	if (cfi->cfiq->NumEraseRegions == 1) {
		...
		for (i=0; i<cfi->cfiq->NumEraseRegions; i++) {
			...
			unsigned long cfi_data=0;
			__u32 base = cfi->chips[0].start;
			int ofs_factor = cfi->interleave * cfi->device_type;

			ersize = ((cfi->cfiq->EraseRegionInfo[i] >> 8) & ~0xff) * cfi->interleave;
			ernum = (cfi->cfiq->EraseRegionInfo[i] & 0xffff) + 1;
			printk("Region%d: SectorSize=0x%x SectorNum=%d\n", i, ersize, ernum);
			...
			for (j=0; j<cfi->numchips; j++) {
			    /*
			     * User should write the reset command twice then device can return to read state 
	...
	case 8:
		...
		if ( cfi->cfiq->BufWriteTimeoutTyp && !FORCE_WORD_WRITE) {
		        printk("cfi_cmdset_0002: Using Write Buffer method.\n");
			mtd->write = cfi_amdstd_write_buffers;
		} else {
		        printk("cfi_cmdset_0002: Using word write method.\n");
...
static int do_poll(struct map_info *map, struct flchip *chip, unsigned long adr)
{
	...
	dq6 = CMD(1<<6);
	dq5 = CMD(1<<5);
	timeo = jiffies + (HZ/1000); /* setting timeout to 1ms for safety */
...
static int do_write_oneword(struct map_info *map, struct flchip *chip, unsigned long adr, __u32 datum, int fast)
{
 ...
 retry:
	...
	while( ((status & dq6) != (oldstatus & dq6) && 
	       (status & dq5) != dq5 &&
	       !time_after(jiffies, timeo)) ||
	       (cfi_read(map, adr) & 0x80) != (datum & 0x80)) {
