...
hash_conntrack(const struct ip_conntrack_tuple *tuple)
{
  ...
  if ( tuple->dst.protonum == IPPROTO_GRE ) {
	return (jhash_3words(tuple->src.ip + tuple->dst.ip,
	                     (tuple->dst.protonum),
	                     (tuple->src.u.all),
	                     (tuple->dst.u.all << 16)) % ip_conntrack_htable_size);
  }
...
static inline int
conntrack_cone_tuple_cmp(const struct ip_conntrack_tuple_hash *i,
		    const struct ip_conntrack_tuple *tuple,
		    const struct ip_conntrack *ignored_conntrack)
{
    MUST_BE_READ_LOCKED(&ip_conntrack_lock);
    return i->ctrack != ignored_conntrack
	&& ip_ct_cone_tuple_equal(tuple, &i->tuple);
}
